...
xlr {
  template('4.01 EjemploGlobal') {
    ...
    scriptUserPassword '{aes:v0}oINB4/LkdKdEB8YQJtLp8axG9TjFNrGpiCIubJRqXtE='
    phases {
      phase('BUILD') {
        color '#0099CC'
        tasks {
      ...
      phase('TEST Y VALIDACIÓN') {
        ...
        tasks {
          ...
          manual('Ejecución de pruebas manuales') {
            description '### Pruebas manuales\n' +
                        '\n' +
                        'Deben iniciarse las pruebas manuales para la validación del microservicio desplegado\n' +
                        '\n' +
                        '#### Como resultado debe existir un documento con el resultado de las mimas en la siguiente URL\n' +
                        '\n' +
                        'http://localhost/test/results/${microservice-name}/${ms-version}'
            team 'Desarrollo'
          }
